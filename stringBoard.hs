...
import Debug.Trace -- ska hjälpa med debugging
...
--possibleColor gP board color = undefined                      --possibleColor (getPosition board color) (oppositeColor color) board
    --if getCell board (map (x-1, _)(getPosition board color)) == oppositeColor color     ----- ([(x-1, y) | (x,y) <- head gP]) == oppositeColor color 
       --then getPosition board color
--else [(1,1)]
--    |
--    |
--    |
...
-- funktion för fallen
getNeighbour :: Playfield -> Pos -> Pos
getNeighbour board ppos = undefined
...
-- https://github.com/LucidThought/OthelloHaskell/blob/master/OthelloTools.hs
